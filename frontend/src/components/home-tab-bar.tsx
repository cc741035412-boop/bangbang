...
const TABS = [
  { key: "today", label: "今日素材", path: "/", icon: Inbox },
  { key: "month", label: "本月素材", path: "/month", icon: Folder },
  { key: "mine", label: "我的", path: "/mine", icon: UserRound },
] as const;
      ...
      {TABS.map((tab) => {
        ...
        return (
          <Link
            ...
            className={`flex min-h-14 flex-col items-center justify-center gap-1 text-xs font-medium ${selected ? "text-brand" : "text-[#99a29e]"}`}
            key={tab.key}
            to={tab.path}
          >
